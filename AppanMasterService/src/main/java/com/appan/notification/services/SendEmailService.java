...
public class SendEmailService {
	...
	public CommonResponse sendEmail(@Valid SendSmsRequest req) {
		...
		try {
			...
			if (Strings.isNullOrEmpty(req.getEmailId())) {
				response.setStatus(false);
				response.setMessage("EmailID is required");
				response.setRespCode("01");
				return response;
			}

			processMobileNumbers(req.getEmailId(), req);

			response.setStatus(true);
			response.setMessage("Email request accepted successfully.");
	...
	public CommonResponse bulkEmail(@Valid SendSmsRequest req) {
		CommonResponse response = new CommonResponse();
		...
		try {
			...
			response.setStatus(true);
			response.setMessage("Bulk Email request accepted successfully.");
			response.setRespCode("00");
		} catch (Exception e) {
			logger.error("Exception occurred while creating wallet: ", e);
			response.setStatus(false);
