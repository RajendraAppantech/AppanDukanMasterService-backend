...
@Service
public class EmailTemplateAuthOrBlockService {
	...
	private static final Logger logger = LoggerFactory.getLogger(EmailTemplateAuthOrBlockService.class);

	public CommonResponse authorblock(@Valid EmailTemplateAuthOrBlockRequest req) {
		...
		try {
			...
			EmailTemplateMaster mst = repository.findById(req.getId()).orElse(null);
			if (mst == null) {
				response.setStatus(false);
			...
			if (req.getStatus().equalsIgnoreCase("1") && currentStatus.equalsIgnoreCase("1")) {
				response.setStatus(false);
				response.setMessage("Email Template is already authorized.");
				response.setRespCode("01");
				return response;
			}
			...
			if (req.getStatus().equalsIgnoreCase("3") && currentStatus.equalsIgnoreCase("3")) {
				response.setStatus(false);
				response.setMessage("Email Template is already blocked.");
				response.setRespCode("01");
				return response;
			}
			...
			if (req.getStatus().equalsIgnoreCase("1")) {
				mst.setAuthStatus("1");
				response.setMessage("Email Template authorized successfully.");
			} else if (req.getStatus().equalsIgnoreCase("3")) {
				mst.setAuthStatus("3");
				response.setMessage("Email Template blocked successfully.");
			} else {
				response.setStatus(false);
