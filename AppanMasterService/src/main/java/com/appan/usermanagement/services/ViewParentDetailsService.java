...
@Service
public class ViewParentDetailsService {
	...
	private static final Logger logger = LoggerFactory.getLogger(ViewParentDetailsService.class);

	public UserManagementFetchResponse viewParent(UserManagementFetchRequest req, Integer pageNo, Integer pageSize) {
		...
		try {
			...
			if (Strings.isNullOrEmpty(req.getParent())) {
				return buildErrorResponse(false, "Parent ID cannot be null/Empty", "01", null, null);
			}

			Pageable paging = PageRequest.of(pageNo - 1, pageSize);

			Specification<UserManagementMaster> specification = (root, query, criteriaBuilder) -> {
				List<Predicate> predicates = buildPredicates(criteriaBuilder, root, req.getParent());
	...
	private List<Predicate> buildPredicates(CriteriaBuilder cb, Root<UserManagementMaster> root, String parent) {
		List<Predicate> predicates = new ArrayList<>();
		predicates.add(cb.equal(cb.lower(root.get("userId")), parent.toLowerCase()));
		return predicates;
	}
