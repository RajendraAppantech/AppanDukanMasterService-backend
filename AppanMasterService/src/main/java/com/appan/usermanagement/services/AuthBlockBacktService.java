...
@Service
public class AuthBlockBacktService {
	...
	private static final Logger logger = LoggerFactory.getLogger(AuthBlockBacktService.class);

	public CommonResponse authBlock(@Valid AuthOrBlockBackRequest req) {
		...
		try {
			...
			BackOfficeManagement mst = repository.findById(req.getId()).orElse(null);
			if (mst == null) {
				response.setStatus(false);
				response.setMessage("Back office with the given ID not found.");
				response.setRespCode("01");
				return response;
			}
			...
			if (req.getStatus().equalsIgnoreCase("1") && currentStatus.equalsIgnoreCase("1")) {
				response.setStatus(false);
				response.setMessage("Back office is already authorized.");
				response.setRespCode("01");
				return response;
			}
			...
			if (req.getStatus().equalsIgnoreCase("3") && currentStatus.equalsIgnoreCase("3")) {
				response.setStatus(false);
				response.setMessage("Back office is already blocked.");
				response.setRespCode("01");
				return response;
			}
			...
			if (req.getStatus().equalsIgnoreCase("1")) {
				mst.setAuthStatus("1");
				response.setMessage("Back office authorized successfully.");
			} else if (req.getStatus().equalsIgnoreCase("3")) {
				mst.setAuthStatus("3");
				response.setMessage("Back office blocked successfully.");
			} else {
				response.setStatus(false);
