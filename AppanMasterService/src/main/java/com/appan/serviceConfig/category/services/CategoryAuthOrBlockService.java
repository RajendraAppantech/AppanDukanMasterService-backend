...
@Service
public class CategoryAuthOrBlockService {
	...
	private static final Logger logger = LoggerFactory.getLogger(CategoryAuthOrBlockService.class);

	public CommonResponse authorblock(@Valid CategoryAuthOrBlockRequest req) {
		...
		try {
			...
			ServiceConfigCategoryMaster mst = repository.findById(req.getId()).orElse(null);
			if (mst == null) {
				response.setStatus(false);
			...
			if (req.getStatus().equalsIgnoreCase("1") && currentStatus.equalsIgnoreCase("1")) {
				response.setStatus(false);
				response.setMessage("Service Category is already authorized.");
				response.setRespCode("01");
				return response;
			}
			...
			if (req.getStatus().equalsIgnoreCase("3") && currentStatus.equalsIgnoreCase("3")) {
				response.setStatus(false);
				response.setMessage("Service Category is already blocked.");
				response.setRespCode("01");
				return response;
			}
			...
			if (req.getStatus().equalsIgnoreCase("1")) {
				mst.setAuthStatus("1"); 
				response.setMessage("Service Category authorized successfully.");
			} else if (req.getStatus().equalsIgnoreCase("3")) {
				mst.setAuthStatus("3"); 
				response.setMessage("Service Category blocked successfully.");
			} else {
				response.setStatus(false);
